...
import type { ISpriteProvider } from '../rendering/ISpriteProvider';
import { KenneySpriteProvider } from '../sprites/KenneySpriteProvider';
...
export class AssetLoader {
  private static spriteProvider: ISpriteProvider | null = null;

  static getSpriteProvider(): ISpriteProvider {
    if (!AssetLoader.spriteProvider) {
      throw new Error('Sprite provider not initialized. Call loadAll() first.');
    }
    return AssetLoader.spriteProvider;
  }
  ...
  static async loadAll(): Promise<GameAssets> {
    // Initialize sprite provider
    const provider = new KenneySpriteProvider();
    await provider.init();
    AssetLoader.spriteProvider = provider;

    // Load map data
    const mapData = await AssetLoader.loadMapData(DEFAULT_MAP_DATA_PATH);

    // Also load the legacy spritesheet for backward compatibility with existing renderers
    const [spritesheetTexture] = await Promise.all([
   ...
   */
  static async loadMapData(path: string): Promise<MapData> {
  ...
  private static sliceSpritesheet(baseTexture: Texture): Map<number, Texture> {
    ...
    for (let row = 0; row < SHEET_ROWS; row++) {
      for (let col = 0; col < SHEET_COLUMNS; col++) {
        ...
        console.log(`Slicing tileId ${tileId} at grid (${col}, ${row})`);
