import type { MapData } from '../core/AssetLoader';
...
export class TileMap {
    ...
    private moveEntity(fromCol: number, fromRow: number, toCol: number, toRow: number, layerName: string): boolean {
    ...
    public moveUnit(fromCol: number, fromRow: number, toCol: number, toRow: number): boolean {
        return this.moveEntity(fromCol, fromRow, toCol, toRow, 'Vehicles');
    }
    ...
    isOccupied(col: number, row: number): boolean {
        if (!this.inBounds(col, row)) {
            console.warn(`isOccupied check out of bounds at (${col}, ${row})`);
            return false; // Out of bounds
        }
        if (this.getTile(col, row, 'Vehicles') !== 0) {
    ...
    isPassable(col: number, row: number): boolean {
        if (!this.inBounds(col, row)) {
            console.warn(`isPassable check out of bounds at (${col}, ${row})`);
            return false; // Out of bounds
        }
        if (this.getTile(col, row, 'Terrain') === 0) {
